...
let gameLoop = () => {
    ...
    circle.setAttribute("transform", `translate(${movement.circle().screenX} ${movement.circle().y})`);
    layer3.setAttribute("transform", `translate(${movement.scene().x} 0)`); 
    layer2.setAttribute("transform", `translate(${0.6 * movement.scene().x} 0)`); 
    layer1.setAttribute("transform", `translate(${0.2 * movement.scene().x} 0)`); 

    let line = scene.findLine(movement.circle().x);
    ...
    console.log(`${movement.circle().x} - circle: ${movement.circle().screenX}, scene: ${movement.scene().x}`);
}
